...
@Component({
  ...
  template: `
    <a
      class="flex flex-wrap items-center gap-x-5 gap-y-1 border-b border-outline-variant bg-surface px-4 py-3.5 outline-offset-[-3px] hover:bg-surface-container focus-visible:bg-surface-container focus-visible:outline-3 focus-visible:outline-primary"
      [routerLink]="link()"
    >
      <span class="w-full text-title-large text-on-surface sm:w-[130px]">{{ time() }}</span>
